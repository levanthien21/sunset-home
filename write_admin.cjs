...
export default function AdminRooms() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("C?p nh?t phòng thành công!");
      setEditingRoom(null);
      fetchRooms();
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="p-8 font-sans max-w-6xl mx-auto">
      <div className="flex justify-between items-end mb-8">
        <div>
          <h1 className="text-3xl font-serif text-gray-900 mb-2">Qu?n lý Phòng & Giá</h1>
          <p className="text-gray-500">Thi?t l?p c?u hình phòng, giá các gói theo gi? và qua dêm.</p>
        </div>
      </div>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 uppercase font-bold tracking-wider">{room.branch_id === 1 ? "Chi nhánh B?n L?c" : "Chi nhánh H?u Nghia"}</p>
                  </div>
                </div>
            ...
            <div className="p-6 overflow-y-auto flex-1">
              <form id="edit-room-form" onSubmit={handleSave} className="space-y-6">
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-500 mb-2 uppercase">Giá ph? thu gi? (VNÐ)</label>
                    <input 
                      required 
                      type="number" 
                ...
                <div>
                  <div className="flex justify-between items-center mb-4">
                    <label className="block text-xs font-bold text-gray-500 uppercase">Các gói gi? (Combos)</label>
                    <button 
                      type="button"
                      onClick={() => setEditingRoom({...editingRoom, combos: [...(editingRoom.combos || []), { id: "combo-" + Date.now(), name: "Gói M?i", price: 0 }]})}
